...
export const ArtMenu = () => {
  ...
  const nextModalInfo = useMemo<ModalInfo | null>(() => {
    ...
    for (let i = 0; i < itemsMenu.length; i++) {
      if (
        itemsMenu[i].modalInfo.title === modalInfo.title &&
        itemsMenu[i + 1]
      ) {
        nextModalInfo = itemsMenu[i + 1].modalInfo;
        break;
      }
    }
          ...
          scrollContent={
            <div className={styles.cardList}>
              {modalInfo.listImages.map((item, id) => (
                <GlassCard key={id} type={item.type} imageUrl={item.src} />
              ))}
            </div>
          }
        />
